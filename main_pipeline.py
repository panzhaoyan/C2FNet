...
parser.add_argument("--fusion_t_in", type=int, default=192)
parser.add_argument("--fusion_t_hid", type=int, default=128)
parser.add_argument("--fusion_a_in", type=int, default=192)
parser.add_argument("--fusion_a_hid", type=int, default=128)
parser.add_argument("--fusion_v_in", type=int, default=192)
parser.add_argument("--fusion_v_hid", type=int, default=128)
parser.add_argument("--fusion_gru_layers", type=int, default=1)
parser.add_argument("--fusion_drop", type=float, default=0.2)
parser.add_argument("--cls_hidden_dim", type=int, default=128)
parser.add_argument("--cls_dropout", type=float, default=0.2)
parser.add_argument("--fusionModule", type=str, default="c_gate")
...
parser.add_argument("--learning_rate_a", type=float, default=0.0013)
parser.add_argument("--learning_rate_h", type=float, default=0.00000025)
parser.add_argument("--learning_rate_v", type=float, default=0.0013)
...
def prep_for_training(num_training_steps):
    ...
    if args.model == "our":
        ...
        visual_model_ctf = Transformer(VISUAL_DIM, num_layers=8, nhead=4, dim_feedforward=1024)
        visual_model_ctf.load_state_dict(torch.load("./model_weights/init/sarcasm/sarcasmVisualTransformer.pt"))
        ...
        acoustic_model_ctf = Transformer(ACOUSTIC_DIM, num_layers=1, nhead=3, dim_feedforward=512)
        acoustic_model_ctf.load_state_dict(torch.load("./model_weights/init/sarcasm/sarcasmAcousticTransformer.pt"))
    
        text_model = AlbertModel.from_pretrained('./albert-base-v2')
        text_model_ctf = AlbertModel.from_pretrained('./albert-base-v2')
    
        model = Ours(
            text_model, text_model_ctf,
            visual_model, visual_model_ctf,
            acoustic_model, acoustic_model_ctf,
            args, fusion_dim=args.fusion_dim
        )
    else:
