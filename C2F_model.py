...
class Ours(nn.Module):
    def __init__(self, text_model, text_model_ctf, visual_model, visual_model_ctf,
                 acoustic_model, acoustic_model_ctf, args, fusion_dim=128):
        super(Ours, self).__init__()

        self.text_model = text_model
        self.text_model_ctf = text_model_ctf
        self.tf = nn.Linear(args.text_dim, fusion_dim)
        self.tf_ctf = nn.Linear(args.text_dim, fusion_dim)

        self.visual_model = visual_model
        self.visual_model_ctf = visual_model_ctf
        self.vf = nn.Linear(visual_model.d_model, fusion_dim)
        self.vf_ctf = nn.Linear(visual_model_ctf.d_model, fusion_dim)

        self.acoustic_model = acoustic_model
        self.acoustic_model_ctf = acoustic_model_ctf
        self.af = nn.Linear(acoustic_model.d_model, fusion_dim)
        self.af_ctf = nn.Linear(acoustic_model_ctf.d_model, fusion_dim)

        self.fusion = Fusion(args)
        self.feature_proj = nn.Linear(448, 192)  # ✅ 注册为模块成员

        fusion_layer = CrossAttentionLayer(
            fusion_dim, fusion_dim, fusion_dim,
            nhead=args.cross_n_heads, dropout=args.dropout
        )
        self.modal_fusion = FusionEncoder(
            fusion_layer, args.modal_n_layers, fusion_dim,
            nhead=args.cross_n_heads, dropout=args.dropout, modal='sentiment'
        )
    ...
    def get_params(self):
        acoustic_params = list(self.acoustic_model.named_parameters()) + \
                          list(self.acoustic_model_ctf.named_parameters())
        visual_params = list(self.visual_model.named_parameters()) + \
                        list(self.visual_model_ctf.named_parameters())
        other_params = (
            list(self.text_model.named_parameters()) +
            list(self.text_model_ctf.named_parameters()) +
            list(self.tf.named_parameters()) + list(self.tf_ctf.named_parameters()) +
            list(self.vf.named_parameters()) + list(self.vf_ctf.named_parameters()) +
            list(self.af.named_parameters()) + list(self.af_ctf.named_parameters()) +
            list(self.feature_proj.named_parameters()) +
            list(self.modal_fusion.named_parameters()) +
            list(self.se.named_parameters())
        )
    ...
    def forward(self, input_ids, visual, acoustic, attention_mask=None, token_type_ids=None):
        ...
        text_output_ctf = self.tf_ctf(self.text_model_ctf(input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids)[0])
        acoustic_output_ctf = self.af_ctf(self.acoustic_model_ctf(acoustic)[2])
        visual_output_ctf = self.vf_ctf(self.visual_model_ctf(visual)[2])
        ...
        text_mask = torch.zeros_like(attention_mask).to(text_output.device)
        acoustic_mask = torch.zeros_like(attention_mask).to(acoustic_output.device)
        visual_mask = torch.zeros_like(attention_mask).to(visual_output.device)

        text_input = (text_output_ctf, text_mask)
        acoustic_input = (acoustic_output_ctf, acoustic_mask)
        visual_input = (visual_output_ctf, visual_mask)

        feature_fusion = self.fusion(text_input, acoustic_input, visual_input)  # [B, 448]
        feature_fusion = self.feature_proj(feature_fusion)  # [B, 192]

        L = text_output.size(1)
        feature_fusion = feature_fusion.unsqueeze(1).expand(-1, L, -1).contiguous()

        modal_fusion = self.modal_fusion(
            text_output, visual_output, acoustic_output,
            feature_fusion, attention_mask=extended_attention_mask
        )
        sentiment_embedding = F.max_pool1d(modal_fusion.permute(0, 2, 1), kernel_size=L).squeeze(-1)
        return self.se(sentiment_embedding)
